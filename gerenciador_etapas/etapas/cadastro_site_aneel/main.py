...
class CadastroSiteAneel:
    ...
    def tratamento_dos_dados_da_tupla(self, tupla):
        ...
        tupla['endereco_da_uc'] = f"{tupla['endereco']}"
    ...
    def tratamento_da_nota(self, tupla_notas: dict, driver):
        self.tratamento_dos_dados_da_tupla(tupla=tupla_notas)
        try:
            titulo_da_pagina = driver.current_url
            if 'links' in titulo_da_pagina:
        ...
        if 'Inclui_Usina' in titulo_da_pagina:
            ...
            telefone = tupla_notas['tel_movel']

            email = [var for var in tupla_notas['email'].split(';')]

            self.static_site_aneel.formulario_cadastro(driver, tupla_notas['municipio'],
                                tupla_notas['uc'],
                                tupla_notas['qtd_gd'], 
                                tupla_notas['endereco_da_uc'],
                                tupla_notas['cep'],
                                email[0], tupla_notas['nome_cliente'], cpf_cnpj,
                                telefone, tupla_notas['modalidade'],
        ...
        try:
            ...
            texto_alert = driver.switch_to.alert.text
        ...
        if 'Inclui_Dados' in titulo_da_pagina:
            # print(tupla_notas)
            self.static_site_aneel.formulario_tecnico(driver, tupla_notas['pot_modulos_kwp'], tupla_notas['quantidade_modulos'], tupla_notas['pot_inversores_kwp'], tupla_notas['qtde_inversores'],
                               tupla_notas['area_arranjo'], tupla_notas['fabricante_modulos'],
            ...
            msg_erro = driver.find_element(by=By.CLASS_NAME, value='mensagemErro').text
        ...
        if 'Inclui_CEGanterior' in titulo_da_pagina:
            self.static_site_aneel.tel_dados_usina(driver)
        else:
            raise AssertionError(
        ...
        if 'Conclusao' in titulo_da_pagina:
            return self.static_site_aneel.tela_para_obtencao_codigo_gd(driver, empresa_abreviado=self.empresa_abreviado)
        else:
            raise AssertionError(
                f'Página não contem a string Inclui_CEGanterior, como era esperado: {titulo_da_pagina}')

    def run(self, id_nota, driver_aneel):
        ...
        if len(lista_notas) == 1:
            ...
            codigo_gd_final = self.tratamento_da_nota(tupla_notas, driver_aneel)
    ...
    def iniciar_navegador(self):
        try:
            driver.get('http://www2.aneel.gov.br/scg/gd/login.asp')
        except:
            chrome_options = webdriver.ChromeOptions()
            ...
            driver.get('http://www2.aneel.gov.br/scg/gd/login.asp')
            self.static_site_aneel.login(driver, self.usuario_aneel, self.senha_aneel)

        return driver
    
    def verificar_cadastro(self, id_nota, driver):
        self.conexao_datamart.atualizar_registro(dicionario={'tempo_criacao': datetime.datetime.now()},
                                        tabela=__class__.tabela, id_=id_nota)
        ...
        try:
            ...
            if 'links' in titulo_da_pagina:
                ...
                for elemento in lista_de_elementos_botao:
                    ...
                    raise AssertionError('Não foi possível encontrar o elemento com o nome "Ver, alterar, incluir fonte ou excluir UC com GD cadastradas" na tela de menu')
            else:
                raise AssertionError(f'Titulo da página é diferente de links :|: {titulo_da_pagina}')

            titulo_da_pagina = driver.current_url
            if 'adm_empreendimento' not in titulo_da_pagina:
                raise AssertionError(f'Página não contem a string adm_empreendimento, como era esperado: {titulo_da_pagina}')

            tupla_uc = self.conexao_datamart.consultar_notas(sql=sql_consulta_uc)
            # print(tupla_uc[0]['uc'])

            resultado_funcao = self.static_site_aneel.consultar_registro(driver=driver, uc=tupla_uc[0]['uc'])
            if 'Incompleta' not in resultado_funcao and resultado_funcao != 'Não cadastrado':
                split_4 = resultado_funcao.split(' ')
                resultado_funcao = split_4[len(split_4) - 3]
                self.logger_nota.critical(f'Status do registro na ANEEL é diferente do padrão :|: {resultado_funcao}')
                print(resultado_funcao)
                raise ErroPrevisto(mensagem= 'Cliente com cadastro Completo/Incompleto')
            ...
            if resultado_funcao == 'Não cadastrado':
                ...
                if 'links' in titulo_da_pagina:
                    ...
                    for elemento in lista_de_elementos_botao:
                        titulo_do_elemento = elemento.get_attribute('title')
                        if titulo_do_elemento == 'Cadastrar nova UC com GD (Unidades Consumidoras com Geração Distribuída)':
                            elemento.click()
                            break
                    else:
                        raise AssertionError(f'Página não contem a string links, como era esperado: {titulo_da_pagina}')
                    
        except AssertionError as excecao:
            self.logger_nota.critical(excecao.mensagem, str(type(excecao)), excecao.valor)
            self.conexao_datamart.atualizar_registro(
                dicionario={
                    'repetir': False,
